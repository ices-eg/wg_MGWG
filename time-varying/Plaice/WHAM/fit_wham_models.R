...
# source("/home/bstock/Documents/wg_MGWG/time-varying/Plaice/WHAM/fit_wham_models.R")
...
# get m10 par to start m9
m10 <- readRDS("/home/bstock/Documents/wham/sandbox/ices_selectivity/Plaice/run3_fix67/m10.rds")
m10_logit_selpars <- m10$parList$logit_selpars
m10_sel_repars <- m10$parList$sel_repars
rm("m10")
...
for(i in 1:n.mods){
	...
	input <- prepare_wham_input(asap3, model_name = paste0(model.id," m",m,": NAA = ",df$NAA_mod[i],", sel = ",df$sel_mod[i]," (",df$sel_re[i],")"),
						selectivity=list(model=c(df$sel_mod[i],"age-specific","age-specific","age-specific","age-specific"), re=c(df$sel_re[i],"none","none","none","none"),
										initial_pars=list(sel_inits, c(0.5,0.5,0.5,1,0.5,0.5,0.5,0.5,0.5,0.5,0.5), c(0.5,0.5,0.5,0.5,1,0.5,0.5,0.5,0.5,0.5,0.5), c(0.5,0.5,1,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5), c(0.5,0.5,1,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5)),
										fix_pars=list(fix_pars, 4, 5, 3, 3)))
	...
	if(m == 9){
		input$par$logit_selpars <- m10_logit_selpars
		input$par$sel_repars[1,c(1,3)] <- m10_sel_repars[1,c(1,3)]
	}	
