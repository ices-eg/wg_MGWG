...
# source("/home/bstock/Documents/wg_MGWG/time-varying/CCGOMyt/WHAM/fit_wham_models.R")
...
# for(m in 1:5){	# only logistic
	...
	input <- prepare_wham_input(asap3, model_name = paste0(model.id," m",m,": NAA = ",df$NAA_mod[m],", sel = ",df$sel_mod[m]," (",df$sel_re[m],")"),
						selectivity=list(model=c(df$sel_mod[m],"logistic","age-specific","logistic","age-specific"), re=c(df$sel_re[m],"none","none","none","none"),
										initial_pars=list(sel_inits, sel_inits_logistic, c(0.5,1,1,0.5,0.5,0.5), sel_inits_logistic, c(1,1,1,0.5,0.5,0.5)),
										fix_pars=list(fix_pars, NULL, 2:3, NULL, 1:3)))
