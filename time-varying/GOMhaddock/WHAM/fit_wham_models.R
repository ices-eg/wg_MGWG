...
# source("/home/bstock/Documents/wg_MGWG/time-varying/GOMhaddock/WHAM/fit_wham_models.R")
...
runmods <- 1:10 # all
# runmods <- 6:10 # age-specific
# runmods <- c(1,6) # time-constant	
# runmods <- 1:5) # logistic
# runmods <- c(3,6:10)
...
# fix_pars_age <- c(1,6:7)
# m6 <- readRDS("/home/bstock/Documents/wham/sandbox/ices_selectivity/GOMhaddock/run4_fix67/m6.rds")
# sel_inits_age[fix_pars_age] = c(plogis(m6$rep$logit_selpars[1,1]),1,1)
...
# m2 <- readRDS("/home/bstock/Documents/wham/sandbox/ices_selectivity/GOMhaddock/run2_index348_index368/m5.rds")
# m2_logit_selpars <- m2$parList$logit_selpars
# m2_sel_repars <- m2$parList$sel_repars
# rm("m2")

for(i in 1:n.mods){
	m = runmods[i]
	if(df$sel_mod[i] == "age-specific"){
	...
	input <- prepare_wham_input(asap3, model_name = paste0(model.id," m",m,": NAA = ",df$NAA_mod[i],", sel = ",df$sel_mod[i]," (",df$sel_re[i],")"),
						selectivity=list(model=c(df$sel_mod[i],"age-specific","age-specific"), re=c(df$sel_re[i],"none","none"),
										initial_pars=list(sel_inits, c(0.5,0.5,1,1,0.5,0.5,0.5,1,0.5), c(0.5,0.5,1,0.5,0.5,1,0.5,1,0.5)),
										fix_pars=list(fix_pars, c(3,4,8), c(3,6,8))))
	...
	# if(m %in% 7:10) input$par$logit_selpars = inits_logit_selpars	

	# if(m == 3){
	# 	input$par$logit_selpars <- m2_logit_selpars
	# 	input$par$sel_repars[1,1:2] <- m2_sel_repars[1,1:2]
	# }
	...
	# NAA option
	if(df$NAA_mod[i] == "rec"){ # recruitment random effects
	...
	df$runtime_min[i] = round(as.numeric(difftime(etime, btime), units="mins"),2)
	df$nll[i] = mod$opt$obj
	df$conv[i] = mod$opt$convergence
	selAA[[i]] <- mod$report()$selAA[[1]]
	conv[[i]] <- capture.output(check_convergence(mod))
	rm("mod")
}
mod.list <- file.path(local.dir,paste0("m",runmods,".rds"))
...
for(i in 1:n.mods){
	m = runmods[i]
	tmp <- as.data.frame(selAA[[i]])
	tmp$Year <- input$years
	colnames(tmp) <- c(paste0("Age_",1:input$data$n_ages),"Year")
	tmp$Model <- paste0("m",m,": NAA = ",df$NAA_mod[i],", sel = ",df$sel_mod[i]," (",df$sel_re[i],")")
	tmp$sel_re <- df$sel_re[i]
	tmp$sel_mod <- df$sel_mod[i]
...
df.plot$Model <- factor(as.character(df.plot$Model), levels=paste0("m",runmods,": NAA = ",df$NAA_mod,", sel = ",df$sel_mod," (",df$sel_re,")"))
