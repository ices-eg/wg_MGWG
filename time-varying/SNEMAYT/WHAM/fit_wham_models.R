...
# source("/home/bstock/Documents/wg_MGWG/time-varying/SNEMAYT/WHAM/fit_wham_models.R")
...
ICES2ASAP(user.wd, user.od, model.id = model.id, ices.id=paste0(model.id,"_"))
...
# sel_inits_age <- c(plogis(-4),0.5,0.5,1,0.5,0.5)
# fix_pars_age <- c(1,4)
...
# for(m in 1:5){	# only logistic
	...
	# input <- prepare_wham_input(asap3, model_name = paste0(model.id," m",m,": NAA = ",df$NAA_mod[m],", sel = ",df$sel_mod[m]," (",df$sel_re[m],")"),
	# 					selectivity=list(model=c(df$sel_mod[m],"age-specific","age-specific"), re=c(df$sel_re[m],"none","none"),
	# 									initial_pars=list(sel_inits, c(0.5,0.5,0.5,1,0.5,0.5), c(0.5,1,0.5,0.5,0.5,1)),
	# 									fix_pars=list(fix_pars, 4, c(2,6))))	
	input$data$Fbar_ages = Fbar.ages
	# if(df$sel_mod[m] == "logistic"){ input$par$sel_repars[1,1] = -1.3; input$par$logit_selpars[1,7] = -0.65}
