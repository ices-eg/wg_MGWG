...
# source("/home/bstock/Documents/wg_MGWG/time-varying/GOMcod/WHAM/fit_wham_models.R")
...
# for(m in 1:5){	# only logistic
	...
	input <- prepare_wham_input(asap3, model_name = paste0(model.id," m",m,": NAA = ",df$NAA_mod[m],", sel = ",df$sel_mod[m]," (",df$sel_re[m],")"),
						selectivity=list(model=c(df$sel_mod[m],"age-specific","age-specific","age-specific"), re=c(df$sel_re[m],"none","none","none"),
										initial_pars=list(sel_inits, c(0.5,0.5,0.5,0.5,0.5,0.5,1,0.5,0.5), c(0.5,0.5,1,1,1,0.5,0.5,0.5,0.5), c(1,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5)),
										fix_pars=list(fix_pars, 7, 3:5, 1)))
