...
lwd <- 2
stocks <- c("Faroe", "Georges Bank", "Greenland", "Gulf of Maine",
...
stdline(w_baltic$Fmort,      lwd, col[12])
...
stocks <- c("Faroe", "Georges Bank", "Greenland", "Gulf of Maine", "Iceland",
            "NAFO 3M", "NE Arctic", "North Sea", "Norway", "W Baltic")
col <- rich.colors(length(stocks))
...
## Catch
plot(NA, xlim=c(1,14), ylim=c(0,0.44), yaxs="i",
...
stdline(w_baltic$Cp,      lwd, col[10])
legend("topright", legend=stocks, bty="n", lty=1, lwd=lwd, col=col,
       inset=0.02, cex=0.8)
box()
