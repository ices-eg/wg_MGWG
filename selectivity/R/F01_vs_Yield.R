...
refs2<-droplevels(subset(refs, Yield<70000))

jpeg("../F01_plots/Yield_perStock_atS_smallYields.jpg", quality=100, width = 700, height = 700)
gap.plot(refs2$stnum, refs2$Yield,  col=c("black", "grey", "blue"), pch=19, gap=c(29000,47000),
         xtics=levels(as.factor(refs$stnum)), xticlab=F, ylab="Yield", xlab="Stocks")
mtext(c(unique(as.character(refs2$stock))), 1, cex=0.8, at=levels(as.factor(refs2$stnum)), line=rep(c(1,2),6), col=rep(1:2,3, each=2))
legend("topleft", c("young S","current S","old S"), pch=19, col=c("black", "grey", "blue"))
dev.off()
...
jpeg("../F01_plots/Std_Yield_perStock_atS_smallYields.jpg", quality=100, width = 700, height = 700)
