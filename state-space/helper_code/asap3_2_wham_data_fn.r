asap3_2_wham_data_fn <- function(asap3, recruit_model = 2)
{
  ...
  for(i in 1:data$n_fleets) data$catch_paa[i,,] = asap3$CAA_mats[[i]][,1:data$n_ages]/apply(asap3$CAA_mats[[i]][,1:data$n_ages],1,sum)
  ...
  for(i in 1:data$n_indices) 
  {
    data$index_paa[i,,] = asap3$IAA_mats[[i]][,3 + 1:data$n_ages]/apply(asap3$IAA_mats[[i]][,3 + 1:data$n_ages],1,sum)
