...
user.wd <- ".\\state-space\\GBhaddock\\"
user.od <- paste0(user.wd, "ASAP\\")
model.id <- "GBhaddock_"

# convert Lowestoft input files to vanilla ASAP
ICES2ASAP(user.wd, user.od, model.id)

# be sure to use correct age range for Freport

# note: index 3 has missing survey information for a number of years that requires setting Neff to 0
