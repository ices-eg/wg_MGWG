...
x$par$logit_selpars[1,2:8] = Inf
x$par$logit_selpars[2,3:8] = Inf
x$par$logit_selpars[3,3:8] = Inf
...
oldbase = base
base$map$F_devs = factor(rep(NA, length(base$par$F_devs)))
m3 <- fit_wham(make_m3(), do.sdrep = FALSE, do.retro = FALSE, do.osa = FALSE)
base = oldbase
base$par = m3$parList
m3 <- fit_wham(make_m3())
